...
observations = readdlm(observation_file) #reading in the lines - May be too big to deal with
positions = observations[:,1]
observations = observations[:,2]
haplotypes = readdlm(haplotypes_file) #reading in the readlines
...
for initial_prob_position in 1:(num_haplotypes^2)
    ...
    most_likely_paths_so_far[initial_prob_position,1] = 1 * log(get_emission_value(string(OBS_ALLELE), string(founder_1, founder_2), IBD, founder_allelic_error, offspring_allelic_error))
...
for observation_number in (start_of_observations_row + 1):(num_SNPs + 1)
    ...
    global prev_HAPS_LINE = haplotypes[observation_number - 1,:]
    global HAPS_LINE = haplotypes[observation_number,:]
    ...
    for haplotype_state in 1:(num_haplotypes^2) #OPTIMIZATION NEEDED
        ...
        for previous_state in 1:(num_haplotypes^2) #OPTIMIZATION NEEDED
            ...
            finding_the_way[previous_state] = most_likely_paths_so_far[previous_state,observation_number - 2]+log(  get_transition_value(previous_state,haplotype_state)) #observation_number - 2 because you have -1 since obs starts at 2 and also -1 since we're checking the previous value
        ...
        most_likely_paths_so_far[haplotype_state, observation_number - 1] = maxval+log(  get_emission_value(string(OBS_ALLELE), string(founder_1,founder_2), IBD, founder_allelic_error, offspring_allelic_error))
...
for haplotype in 1:2
    global line = string(haplotype, "  ", '\t', "1", '\t')
    global prev_entry = output[1][haplotype]
    for row_num in 2:length(output)
        entry = output[row_num][haplotype]
        if entry != prev_entry
            global line = line * string(output[row_num][3], '\t', "-", '\t', "-", prev_entry)
            println(line)
            global line = string(haplotype, "  ",  '\t', output[row_num][3], '\t')
            global prev_entry = entry
        end
    end
end

line = line * string(output[length(output)][1], '\t', "-", '\t', "-", '\t', prev_entry)
println(line)
