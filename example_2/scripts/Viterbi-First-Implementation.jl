...
function get_emission_value(observation::String, typed_prediction::String, IBD, f_error::Float64, t_error::Float64)
    ...
    if observation == "0" #i.e. observation is the reference allele
        ...
        elseif typed_prediction == ".."
            ...
            error("Wrong haplotype file format.")
            # quit()
            return -1
    ...
    elseif observation == "."
        ...
        error("Wrong observation file format.")
        # quit()
        return -1
...
function get_transition_value(previous_state_index::Int64, state_index::Int64, num_haplotypes::Int64, index_offset_of_first_haplotype::Int64)::Float64
    ...
    if previous_state_index == state_index
        return 0.5 #0.9999999999999999999#0.9999999
    elseif (previous_state_tuple[1] == state_tuple[1]) || (previous_state_tuple[2] == state_tuple[2])
        return 0.5 #0.0000000000000000001#0.0000001
    else
        return 0
    end
...
# function read_in_line()
#     observations = String.(split((readline(observation_file))))
#     haplotypes = String.(split(readline(haplotypes_file)))
#     counter = 1
#     while !eof(observation_file)
#         s = String.(split((readline(observation_file))))
#         observations = hcat(observations, s)
#         # println(s)
#         # push!(observations, s)
#         counter += 1
#     end
#     counter = 1
#     while !eof(haplotypes_file)
#         s = String.(split(readline(haplotypes_file)))
#         haplotypes = hcat(haplotypes, s)
#         # push!(haplotypes, String.(split(readline(haplotypes_file))))
#         counter += 1
#     end
#     println(observations[2,1:10])
#     println("Types ", typeof(haplotypes))
#     observations = permutedims(observations)
#     haplotypes= permutedims(haplotypes)
#     println("Types ", typeof(haplotypes))
#     return observations[:,2], haplotypes
# end

# function read_in()
observations = readdlm(observation_file, String)[:,2]
haplotypes = readdlm(haplotypes_file, String)
    # return observations, haplotypes
# end
# @time read_in()
#
...
# haplotypes_file = open(ARGS[2]) #Second argument: vcf file of all the founder haplotypes.
#
# observations, haplotypes = @time read_in()

close(observation_file)
close(haplotypes_file)
println("Observation File: ",ARGS[1])
println("Founder Haplotype File: ", ARGS[2])
